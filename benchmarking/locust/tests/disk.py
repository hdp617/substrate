...
LOCUST_NO_DISK_USER=1 to skip loading this file, leaving boomer as the
sole owner of DiskUser load.

DiskUser exercises the default glutton template's /tmp data dir — on
micro-VM that is the host overlay rootfs upper — through the same
suspend/resume cycle DurdirUser uses against a DurableDir.
"""

import os

if os.environ.get("LOCUST_NO_DISK_USER") != "1":
    ...
    class DiskUser(User):
        ...
        def noop(self) -> None:
            ...
            # does not load this file (LOCUST_NO_DISK_USER=1). Body is
