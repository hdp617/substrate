...
| `atelet.snapshot.size` | atelet | histogram | uncompressed size in bytes of each snapshot image written during checkpoint (labels `file.name`, `ate.template.atespace`, `ate.template.name`, `ate.sandbox.class`) |
...
| `ate.microvm.checkpoint.duration` | ateom-microvm | histogram | micro-VM checkpoint phase breakdown inside ateom (label `ate.microvm.checkpoint.phase`: pause, snapshot, merge, durable_dir, rootfs_upper, teardown, total) |
| `ate.microvm.restore.duration` | ateom-microvm | histogram | micro-VM restore phase breakdown inside ateom (label `ate.microvm.restore.phase`: prep, bundles, upper_join, lowers, durable, tap, vmm_launch, vm_restore, resume, readyz, total) |
